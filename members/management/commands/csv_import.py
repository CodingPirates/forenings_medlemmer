...
class Command(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (
        ...
        make_option('-f', '--date_format', help="Date format (look in sourcecode)", type="int", default=0, metavar='format'),
    )
    args = 'csvfile'
    ...
    def handle(self, *args, **options):
        ...
        for entry in entries:
            ...
            fam, created = family.Family.objects.get_or_create(email=email)

            # lookup person
            try:
                pers = person.Person.objects.get(name=name, family=fam)
