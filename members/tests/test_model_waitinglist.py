...
from members.tests.factories import WaitingListFactory, DepartmentFactory, PersonFactory, TIMEZONE
...
class TestModelWaitinglist(TestCase):
    ...
    def test_waiting_list_is_sorted_by_person_signup_date(self):
        department = DepartmentFactory()
        subscription0 = WaitingListFactory(person__added = datetime(2017, 1, 1, tzinfo=TIMEZONE),
                                     department=department)
        subscription1 = WaitingListFactory(person__added = datetime(2015, 1, 1, tzinfo=TIMEZONE),
                                     department=department)
        subscription2 = WaitingListFactory(person__added = datetime(2019, 1, 1, tzinfo=TIMEZONE),
                                     department=department)
