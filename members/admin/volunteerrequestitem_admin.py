    ...
    def process_volunteer_request(self, request, queryset):
        for obj in queryset:
            if obj.volunteer_request.person:
                ...
                # Check if there are other volunteerrequestitems with the same volunteerrequest id and status = 1. If none found then set the finished field to now()
                other_requests = VolunteerRequestItem.objects.filter(
